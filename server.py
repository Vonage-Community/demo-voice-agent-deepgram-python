...
from vonage_handlers import handle_vonage, build_ncco
...
async def socket(vonage_ws: WebSocket, original_uuid: str = Query(None)):
    ...
    try:
        ...
        async with websockets.connect(
            ...
            await deepgram_ws.send(json.dumps(DG_AGENT_SETTINGS))
            state.deepgram_websocket_open = True
            logger.info("Deepgram WebSocket open and configured")

            await asyncio.gather(
                handle_deepgram(deepgram_ws, vonage_ws, original_uuid, state),
                handle_vonage(vonage_ws, deepgram_ws, state),
            )

    except Exception as exc:
        logger.error(f"WebSocket bridge error: {exc}")
    finally:
        state.deepgram_websocket_open = False
        logger.info(f"WebSocket session ended | UUID: {original_uuid}")
